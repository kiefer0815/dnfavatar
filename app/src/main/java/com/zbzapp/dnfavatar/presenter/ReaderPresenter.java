...
import com.zbzapp.dnfavatar.manager.ImageSrcManager;
import com.zbzapp.dnfavatar.manager.SourceManager;
import com.zbzapp.dnfavatar.model.*;
...
public class ReaderPresenter extends BasePresenter<ReaderView> {
    ...
    private ImageSrcManager mImageSrcManager;
    ...
    protected void onViewAttach() {
        ...
        mImageSrcManager = ImageSrcManager.getInstance(mBaseView);
    }
    ...
    public void loadInit(long id, Chapter[] array) {
        ...
        List<ImageSrc> imageSrcs = mImageSrcManager.listImageSrc(id);
        if(imageSrcs!=null && imageSrcs.size() > 0){
            this.mChapterManger = new ChapterManger(array, 0);
            images(mImageSrcManager.listImageUrls(imageSrcs,array[0].getPath()));
        }else {
                    ...
                    public void call(List<ImageUrl> list) {
                        ...
                        switch (status) {
                            case LOAD_INIT:
                                ...
                                mImageSrcManager.insertImages(mComic.getId(),list);
                                mBaseView.onChapterChange(chapter);
